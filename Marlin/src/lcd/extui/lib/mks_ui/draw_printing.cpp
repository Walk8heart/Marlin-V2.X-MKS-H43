    ...
    if (marlin_state == MF_SD_COMPLETE) {
      if (once_flag == 0) {
        ...
        if((gCfgItems.language == LANG_SIMPLE_CHINESE)||(gCfgItems.language == LANG_COMPLEX_CHINESE))
          sprintf_P((char *)public_buf_t,PSTR("%s%d%d%s%d%d%s%d%d%s"),print_file_dialog_menu.timeConsum,print_time.hours / 10, print_time.hours % 10,print_file_dialog_menu.hourText, print_time.minutes / 10, print_time.minutes % 10,print_file_dialog_menu.minuteText, print_time.seconds / 10, print_time.seconds % 10,print_file_dialog_menu.secondText);
        else
